export const CharacterIdentity = ({ formData, patchForm }) => {
  const pronounDisplay = [formData.subPronoun, formData.objPronoun]
    .filter(Boolean)
    .join("/");
  const ageDisplay = formData.age ? `${formData.age} years old` : "";
      ...
      <label className="cursor-pointer group">
        ...
        <input
          ...
          onChange={(e) => {
            ...
            reader.onload = (e) => patchForm({ portrait: e.target.result });
      ...
      <div className="flex flex-col gap-2 justify-center h-[120px]">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <input
            ...
            placeholder="they"
            value={formData.subPronoun}
            onChange={(e) => patchForm({ subPronoun: e.target.value })}
          />
          <input
            className="w-12 bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 rounded-md text-xs px-2 py-1 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-cyan-400 dark:focus:border-cyan-500 transition-colors"
            placeholder="them"
            value={formData.objPronoun}
            onChange={(e) => patchForm({ objPronoun: e.target.value })}
          />
          {pronounDisplay && (
